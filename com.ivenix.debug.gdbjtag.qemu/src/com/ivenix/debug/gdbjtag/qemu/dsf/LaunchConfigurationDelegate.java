...
package com.ivenix.debug.gdbjtag.qemu.dsf;
...
import com.ivenix.debug.gdbjtag.dsf.AbstractGnuArmLaunchConfigurationDelegate;
import com.ivenix.debug.gdbjtag.dsf.GnuArmServerServicesLaunchSequence;
import com.ivenix.debug.gdbjtag.qemu.Activator;
import com.ivenix.debug.gdbjtag.qemu.Configuration;
...
public class LaunchConfigurationDelegate extends AbstractGnuArmLaunchConfigurationDelegate {
	...
	protected IPath checkBinaryDetails(final ILaunchConfiguration config) throws CoreException {
		...
		if (doStartServer) {
			...
			// or a board name present, otherwise refuse to start.
			String boardName = "";
			String deviceName = "";
			try {
			...
			if (boardName.isEmpty() && deviceName.isEmpty()) {
				throw new CoreException(
						new Status(IStatus.ERROR, Activator.PLUGIN_ID, "Missing mandatory board or device. "
